...
5. **`PATCH /orders/:id/delivery-status`** (rol `seller`, que fusiona venta y entrega) - notifica por Teams y confirma la entrega. No hay un paso previo de asignacion: todas las entregas con pago verificado le salen a todos los vendedores por igual en `GET /orders/my-deliveries` (a pesar del nombre, ya no filtra por vendedor asignado), y cualquiera puede tomar y marcar cualquiera - "tomar" una entrega es simplemente ser quien marca el primer cambio de estado. Si `selfPickup=true`, no se notifica a un tercero (el destinatario es el propio comprador). Al marcar el estado como `DELIVERED`, se envia ademas un correo de confirmacion al comprador (`DeliveryDetail.buyerEmail`) avisando que su compra ya fue entregada; si el envio falla o SMTP no esta configurado, se registra en el log pero la entrega no se bloquea (`MailerService.sendDeliveryConfirmation`).
